...
if (is.null(image_info)) {
  ...
  # Convert named list to a 2-column data frame
  
  image_df <- data.frame(
    Key = names(image_info),
    Value = unlist(image_info, use.names = FALSE),
    row.names = NULL,
    check.names = FALSE
  )
  
  kableExtra::kable(mage_df, caption = "Container Build Metadata (from /usr/local/share/cellatria/image-meta.json)") %>%
    kableExtra::kable_classic(full_width = FALSE, html_font = "Cambria") %>%
    kableExtra::kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"),
                              full_width = FALSE, fixed_thead = TRUE) %>%
    kableExtra::scroll_box(width = "100%", height = "300px")
